def print_flushed(*args, **kwargs):
    kwargs['flush'] = True
    print(*args, **kwargs)
import os, sys, pprint, re, json, pathlib, hashlib, subprocess, glob, tempfile, time
...
def ensureMsysDirs():
    msysDir = os.path.join(libsDir, 'msys64')
    if os.path.exists(msysDir):
        for d in ['tmp', 'dev/shm', 'dev/mqueue', 'etc']:
            path = os.path.join(msysDir, d)
            if not os.path.exists(path):
                try:
                    os.makedirs(path, exist_ok=True)
                    print_flushed(f"Created MSYS2 directory: {path}")
                except:
                    pass
...
# In standalone mode, we don't need special restore logic as it's already in the repo
pass
...
def checkCacheKey(stage):
    ...
    key = keyPath(stage)
    # In standalone repo, we trust the cache key even if source is cleaned
    # if not os.path.exists(os.path.join(stage['directory'], stage['name'])):
    #    return 'NotFound'
    if not os.path.exists(key):
        print_flushed(f"Key NOT FOUND at {key}")
        return 'Stale'
    with open(key, 'r') as file:
        oldKey = file.read().strip()
        newKey = stage['key'].strip()
        if oldKey == newKey:
            return 'Good'
        else:
            print_flushed(f"Key MISMATCH! Old: {oldKey} New: {newKey}")
            return 'Stale'
...
def printCommands(commands):
    print_flushed('---------------------------------COMMANDS-LIST----------------------------------')
    print_flushed(commands, end='')
    print_flushed('--------------------------------------------------------------------------------')
...
def printFreeSpace():
    ...
    print_flushed(f" (Free: {free // (2**30)} GiB) ", end = '', flush=True)

def runStages():
    onlyStages = []
    rebuildStale = (os.environ.get('GITHUB_ACTIONS') == 'true')
    for arg in sys.argv[1:]:
        if arg in options:
            continue
        elif arg == 'silent':
    ...
    for stage in stages:
        ...
        print_flushed(prefix + ': ', end = '', flush=True)
        printFreeSpace()
        ...
        if checkResult == 'Good':
            print_flushed('SKIPPING')
            continue
        elif checkResult == 'NotFound':
            print_flushed('NOT FOUND, ', end='')
        ...
        if checkResult == 'Skip':
            ...
            continue
        clearCacheKey(stage)
        print_flushed('BUILDING:')
        os.chdir(stage['directory'])
        if not run(commands):
            print_flushed(prefix + ': FAILED')
            finish(1)
        writeCacheKey(stage)
        # Commit and push prebuilt libraries incrementally to save github action progress
        if os.environ.get('GITHUB_ACTIONS') == 'true':
            print_flushed(f"Uploading prebuilt {stage['name']} to GitHub to save progress...")
            
            import shutil
            # Cleanup internal .git to avoid submodule issues
            for walk_root, walk_dirs, walk_files in os.walk(rootDir):
                if '.git' in walk_dirs and walk_root != rootDir:
                    shutil.rmtree(os.path.join(walk_root, '.git'), ignore_errors=True)
                    walk_dirs.remove('.git')
...
if customRunCommand:
    ...
    if len(runCommand) == 1 and runCommand[0] == 'shell':
        print_flushed('Preparing interactive mode..')
        if win:
...
stage('openssl3', """
    ...
    cd openssl3
win32:
    perl Configure no-shared no-tests no-asm debug-VC-WIN32 /FS
win64:
    perl Configure no-shared no-tests no-asm debug-VC-WIN64A /FS
winarm:
    perl Configure no-shared no-tests no-asm debug-VC-WIN64-ARM /FS
win:
    nmake build_libs
    mkdir out.dbg
...
win32_release:
    perl Configure no-shared no-tests no-asm VC-WIN32 /FS
win64_release:
    perl Configure no-shared no-tests no-asm VC-WIN64A /FS
winarm_release:
    perl Configure no-shared no-tests no-asm VC-WIN64-ARM /FS
win_release:
    nmake build_libs
    mkdir out
...
depends:patches/build_ffmpeg_win.sh
    ...
    bash -c "mkdir -p /tmp && sed -i '/nv-codec-headers/d' ../patches/build_ffmpeg_win.sh && sed -i '/local.*install/d' ../patches/build_ffmpeg_win.sh && sed -i 's|export PKG_CONFIG_PATH=.*|export PKG_CONFIG_PATH=../local/lib/pkgconfig|' ../patches/build_ffmpeg_win.sh && bash ../patches/build_ffmpeg_win.sh || (cat ffbuild/config.log && exit 1)"
mac:
